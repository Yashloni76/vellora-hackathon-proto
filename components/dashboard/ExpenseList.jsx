    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-end border-b border-border-dark/30 pb-4">
          <h2 className="text-lg font-bold text-primary tracking-tight underline decoration-[#00ff88]/30 underline-offset-8">Unavoidable Expenses</h2>
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-end border-b border-border-dark/30 pb-4">
          <h2 className="text-lg font-bold text-primary tracking-tight underline decoration-red/30 underline-offset-8">Avoidable Expenses</h2>
...
function ExpenseItem({ expense, type }) {
  ...
  return (
    <div className="group bg-card border border-border-dark rounded-xl p-4 flex items-center justify-between hover:bg-[#1a1f1a] transition-all hover:scale-[1.01]">
      <div className="flex items-center gap-4">
        ...
        <div className="w-10 h-10 rounded-lg bg-gray-900 border border-border-dark/50 flex items-center justify-center group-hover:bg-[#00ff8810] transition-colors">
          <Icon size={18} className="text-muted group-hover:text-green-accent transition-colors" />
        </div>

        {/* Info */}
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <h3 className="text-[13px] font-bold text-primary tracking-tight">{expense.title}</h3>
            {/* Tag Pills */}
            {(expense.tag || expense.mood) && (
              <span className={cn(
                "px-2 py-0.5 rounded text-[8px] font-bold tracking-wider leading-none",
                type === "unavoidable" ? {
                  "bg-[#00ff8815] text-green-accent": expense.tag === "ESSENTIAL",
                  "bg-blue-500/15 text-blue-400": expense.tag === "UTILITY",
                  "bg-yellow-500/15 text-yellow-500": expense.tag === "LOGISTICS"
                } : {
                  "bg-[#ff4444] text-primary": expense.mood === "REGRET",
                  "bg-green-accent text-black": expense.mood === "HAPPY",
                  "bg-[#374151] text-primary": expense.mood === "NEUTRAL"
                }
              )}>
      ...
      <div className="flex items-center gap-4">
        <div className="text-right">
          <p className="text-sm font-bold text-primary">₹{expense.amount.toLocaleString("en-IN")}</p>
